# nushell-show [youtube.com/@nushell-prophet](https://www.youtube.com/@nushell-prophet)

Opinionated manuals, random videos about Nushell and materials related to them.

### Table of contents

- [outdated! 001 - Keeping Nushell settings up to date with new releases (outdated after 0.100)](shows/001-keeping-nushell-settings-up-to-date/README.md)
- [002 - How to Install Nushell on macOS: A Step-by-Step Guide for Newbies](shows/002-install-nushell-macos/README.md)
- [outdated! 003 - Version Control for Nushell Configs: Setting Up a Git Repository](shows/003-version-control-nushell-configs/README.md)
- [004 - Setting $env.XDG_CONFIG_HOME](shows/004-setting-xdg-config-home/README.md)
- [005 - nu-history-tools - benchmark your commands usage against of other users](shows/005-nu-history-tools/README.md)
- [006 - Navigating file paths in Nushell using internal functionality, FZF, or Broot](shows/006-navigating-file-paths/README.md)
- [007 - topiary-nushell](shows/007-topiary-demo/README.md)
